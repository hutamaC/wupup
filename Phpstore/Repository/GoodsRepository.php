...
trait GoodsRepository{
  ...
  */
  ...
  |-------------------------------------------------------------------------------
  */
  public static function recommendCategory($tag){
      ...
      $key              = 'goods_category'.$tag;
      if(Cache::has($key)){

        return Cache::get($key);
      }
      if($value = $cls->getGoodsFromDatabase('cat_id',10,$tag)){
  ...
  |-------------------------------------------------------------------------------
  */
  public static function getGoodsFromDatabase($tag,$num,$datavalue=1){

         $self                                          = new static;
         $row                                           = $self->where($tag,$datavalue)->take($num)->get();
