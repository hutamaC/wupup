<script type="text/javascript">
	...
	var catapp = new Vue({
		  ...
		  methods:{
		  		...
		  		//重新获取商品记录
		  		reGetList:function(jewCat='',jewCatName=""){

		  			$.ajax({
		  				url:"{{url('api/category/'.$id)}}",
		  				type:'GET',
		  				data:{search:this.search,brid:this.brid,jewCat:jewCat},
		  				dataType:'json',
		  				success:function(data){
		  					var info = data.data;
		  					var ginfo = info.goods_list;
		  					for(let i=0;i<ginfo.length;i++){

		  						Vue.set(catapp.rows.goods_list,i,ginfo[i]);
		  					}
		  					catapp.rows.goods_list.splice(ginfo.length);
		  					if(jewCat){
		  						catapp.type = jewCatName;
		  					}
		  				}
		  			});
		  		},
		  		...
		  		//选择国家区间
		  		countryGrade:function(country_name,event){

		  			var  that 			= event.currentTarget;
		  			$(that).addClass('active-item').siblings('.price-item').removeClass('active-item');
		  			catapp.rows.country_name 	= country_name;
		  			//执行ajax
		  			this.todoAjax();
		  		},
		  		//清除国家区间
		  		countryGradeDel:function(){
		  		...
		  		todoAjax:function(){
		  			...
		  			$.ajax({
		  				...
		  				type:'GET',
		  				url:"{{url('api/category/'.$id)}}",
		  				data:'param=' + $.toJSON(param)+'&brid='+this.brid+'&search='+this.search,
		  				dataType:'json',
		  				success:function(data){
		  					var  content 			    = data.data;
		  		...
		  		//获取参数
		  		getBrandParam:function(){

		  			var  info 					= {};
		  				 info.brand_id 			= this.brid;
		  				 info.current_page 		= catapp.rows.page.current_page;
		  				 info.country           = catapp.rows.country_name;
		  			return info;
		  		...
		  		doAjax:function(){
		  			...
		  			$.ajax({
		  				...
		  				data:'param=' + $.toJSON(param)+'&brand_id='+this.brid+'&search='+this.search,
		  				dataType:'json',
		  				success:function(data){
		  					var  content 			    = data.data;
		  					//更新需要刷新的数据
